"""Quality factor — 12M return / 60d vol, long top decile, monthly rebalance."""
import pandas as pd
import numpy as np
...
META = {
    ...
    "description": "Long top decile by 12M return / 60d realized vol, equal-weight, monthly rebalance.",
}
...
def _rank_at(data: dict, date) -> pd.DataFrame:
    ...
    for t, df in data.items():
        ...
        if idx < 252:
            continue
        close = df["close"]
        ret_12m = float(close.iloc[idx] / close.iloc[idx - 252] - 1)
        rets = close.pct_change().iloc[idx - 60 + 1: idx + 1]
        vol = float(rets.std()) if len(rets) == 60 else 0
        score = ret_12m / vol if vol > 0 else 0
        rows.append({"ticker": t, "ret_12m": ret_12m, "vol": vol, "score": score})
    if not rows:
        return pd.DataFrame()
